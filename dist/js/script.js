...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const formGET = {
    ...
    // INITIALIZATION 
    init: function () {
      this.forms.forEach(item => this.bindPostData(item)); // this.showThanksModal();
    },
    // 
    ...
    bindPostData: function (form) {
      form.addEventListener('submit', event => {
        ...
        const statusMessage = document.createElement('img'); // statusMessage.classList.add('status');
        ...
        this.postData('http://localhost:3000/menu', json).then(data => data.text()).then(data => {
